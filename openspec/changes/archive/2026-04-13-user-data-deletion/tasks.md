...
- [x] 1.1 在 `app/(app)/settings/page.tsx` 中新增「危險區域」section，顯示「刪除所有對話紀錄」按鈕（紅色背景 `bg-red-500` / 深紅文字）
- [x] 1.2 點擊按鈕後顯示確認對話框（用 `shadcn/ui` Dialog）：標題「確認刪除」、內文「此操作無法復原，所有對話、分析、平靜指數歷史將被永久刪除」、兩個按鈕（取消 / 確認）
- [x] 1.3 確認後呼叫 `DELETE /api/user/data`，顯示 loading 狀態；成功後顯示「已刪除」提示並重新整理，失敗時顯示錯誤訊息

## 2. 後端 DELETE API — `/api/user/data`

- [x] 2.1 建立 `app/api/user/data/route.ts`，實作 `DELETE` handler：
...
- [x] 2.2 為 `app/api/user/data/route.ts` 建立測試 `tests/api/user-data.test.ts`，覆蓋：已授權 DELETE 執行成功、未授權回 401、資料庫操作失敗回 500

## 3. 資料庫清除邏輯

- [x] 3.1 在 `lib/db.ts` 新增 `deleteUserData(userId: string): Promise<void>` 函數：
...
- [x] 4.1 在 `lib/elevenlabs.ts`（或 `lib/elevenlabs-api.ts` 新建）新增 `deleteElevenLabsConversations(conversationIds: string[]): Promise<void>`：
  ...
  - 若全部成功則回傳 void
- [x] 4.2 測試 ElevenLabs 刪除：用 mock 或 stub 模擬 ElevenLabs API，驗證正確的請求被發出
...
- [x] 5.2 執行 `npm run test` 確認所有測試通過

## 6. 文件更新

- [x] 6.1 在 `docs/dev/` 新增或更新「資料隱私」文件，說明：
  - 用戶可透過設定頁刪除所有對話紀錄
  - 刪除操作無法復原
